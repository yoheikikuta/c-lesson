...
int main(int argc, char* argv[]) {
    ...
    static unsigned char expect_buf[ASM_RESULT_BUF_SIZE];
    static unsigned char actual_buf[ASM_RESULT_BUF_SIZE];
    struct Emitter expect_emitter;
    struct Emitter actual_emitter;
    expect_emitter.buf = expect_buf;
    expect_emitter.pos = 0;
    actual_emitter.buf = actual_buf;
    actual_emitter.pos = 0;

    int pos_expect = 0;
    while(fread(&expect_emitter.buf[pos_expect], 1, 1, fp_expect) != NULL) {
        expect_emitter.pos = pos_expect;
        pos_expect++;
    }
    int pos_actual = 0;
    while(fread(&actual_emitter.buf[pos_actual], 1, 1, fp_actual) != NULL) {
    ...
    for (int i = 0; i < pos_expect; i++) {
        if (i % 4 == 0) {
            expect_word = 0x0;
            actual_word = 0x0;
            expect_word += expect_emitter.buf[i];
            actual_word += actual_emitter.buf[i];
        } else if (i % 4 == 1) {
            expect_word += expect_emitter.buf[i] << 8;
            actual_word += actual_emitter.buf[i] << 8;
        } else if (i % 4 == 2) {
            expect_word += expect_emitter.buf[i] << 16;
            actual_word += actual_emitter.buf[i] << 16;
        } else if (i % 4 == 3) {
            expect_word += expect_emitter.buf[i] << 24;
            actual_word += actual_emitter.buf[i] << 24;
