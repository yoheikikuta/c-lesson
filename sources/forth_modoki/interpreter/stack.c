#include <assert.h>
#include <stdio.h>
#include <string.h>
#include "cl_util.h"
#include "stack.h"


static Data_t stack[STACK_SIZE];
static int stack_num = 0;

static void reset_stack() {
    stack_num = 0;
}

int stack_push(Data_t* push_elem) {
    if (stack_num == STACK_SIZE) {
        return 0;
    } else {
        stack[stack_num++] = *push_elem;

        return 1;
    }
}

int stack_pop(Data_t* out_elem) {
    if (stack_num == 0) {
        return 0;
    } else {
        *out_elem = stack[--stack_num];

        return 1;
    }
}

//
// TEST
//

static void test_pop_from_empty() {
    Data_t input = {UNKNOWN, {0}};
    int expect = 0;
    int actual = stack_pop(&input);

    assert(expect == actual);
    reset_stack();
}

static void test_push_to_full() {
    Data_t input = {UNKNOWN, {0}};
    int expect = 0;
    while (stack_num < STACK_SIZE) {
        stack_push(&input);
    }
    int actual = stack_push(&input);

    assert(expect == actual);
    reset_stack();
}

static void test_one_push() {
    Data_t input = {NUMBER, {123}};
    Data_t expect_data = {NUMBER, {123}};
    int expect = 1;

    int actual = stack_push(&input);
    Data_t* actual_data = &stack[0];

    assert(expect == actual);
    assert_two_data_eq(&expect_data, actual_data);
    reset_stack();
}

static void test_one_push_one_pop() {
    Data_t input = {NUMBER, {123}};
    Data_t expect_data_pop = {NUMBER, {123}};
    int expect = 1;

    stack_push(&input);
    Data_t actual_data_pop;
    int actual = stack_pop(&actual_data_pop);

    assert(expect == actual);
    assert_two_data_eq(&expect_data_pop, &actual_data_pop);
    reset_stack();
}

static void test_two_push() {
    Data_t input_1 = {NUMBER, {123}};
    Data_t input_2 = {LITERAL_NAME, .u.name = "add"};
    Data_t expect_data_1 = {NUMBER, {123}};
    Data_t expect_data_2 = {LITERAL_NAME, .u.name = "add"};
    int expect_1 = 1;
    int expect_2 = 1;

    int actual_1 = stack_push(&input_1);
    int actual_2 = stack_push(&input_2);
    Data_t* actual_data_1 = &stack[0];
    Data_t* actual_data_2 = &stack[1];

    assert(expect_1 == actual_1);
    assert(expect_2 == actual_2);
    assert_two_data_eq(&expect_data_1, actual_data_1);
    assert_two_data_eq(&expect_data_2, actual_data_2);

    reset_stack();
}

static void test_two_push_two_pop() {
    Data_t input_1 = {NUMBER, {123}};
    Data_t input_2 = {LITERAL_NAME, .u.name = "add"};
    Data_t expect_data_pop_1 = {NUMBER, {123}};
    Data_t expect_data_pop_2 = {LITERAL_NAME, .u.name = "add"};
    int expect_1 = 1;
    int expect_2 = 1;

    stack_push(&input_1);
    stack_push(&input_2);

    Data_t actual_data_pop_1, actual_data_pop_2;
    int actual_2 = stack_pop(&actual_data_pop_2);
    int actual_1 = stack_pop(&actual_data_pop_1);

    assert(expect_1 == actual_1);
    assert(expect_2 == actual_2);
    assert_two_data_eq(&expect_data_pop_1, &actual_data_pop_1);
    assert_two_data_eq(&expect_data_pop_2, &actual_data_pop_2);

    reset_stack();
}

static void unit_tests() {
    test_pop_from_empty();
    test_push_to_full();
    test_one_push();
    test_one_push_one_pop();
    test_two_push();
    test_two_push_two_pop();

    printf("All unit tests are successfully passed.\n");
}


#if 0
int main() {
    unit_tests();

    return 0;
}
#endif
